...
export class ChatDemoService {
  constructor(
    @InjectRepository(ChatDemoEntity) private chatRepository: Repository<ChatDemoEntity>,
  ...
  async onModuleInit() {
    ...
    // const chats: LinkedInChatType[] = await this.chatRepository.find();
    ...
    //     await this.chatRepository.update({ id }, ct)
...
//     return await this.chatRepository.findOne({ where: { prospection_campaign_id: c_id, prospect_id: p_id } });
//   }

//   async updateChatOne(c: LinkedInChatType) {
//     return await this.chatRepository.update({ id: c.id }, c)
//   }
...
//       const ps_id = ps.id;
//       const res = await this.chatRepository.findOne({ where: { prospect_id: ps_id, prospection_campaign_id: chat.prospection_campaign_id } });
//       if (!res) {
//         var new_ct: LinkedInChatType = chat;
//         new_ct.prospect_id = ps_id;
//         const c = this.chatRepository.create(new_ct);
